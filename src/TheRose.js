...
export default class TheRose extends Component {
    ...
    for (const standing of standings) {
      standingElements.push(
        <div key={standing.name} className="rose-standing-container">
          ...
          <span className="rose-standing-points">{standing.points}</span>
        </div>
      );
    }
  ...
  onNameClick(standing) {
    if (standing.name === (this.state.standing && this.state.standing.name)) {
      this.setState({
        standing: null
      });
    } else {
  ...
  getPlayerElements(players) {
    ...
    for (const player of players) {
      playerElements.push(
        ...
        >
          <span className={player.eliminated && "player-name-eliminated"}>
            {player.name} (pick #{player.pick})
          </span>
          {player.note && <div className="player-note">{player.note}</div>}
          <img
